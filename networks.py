...
class RSSM(tools.Module):
    ...
    def kl_loss(self, post, prior, balance, free, scale):
        ...
        if balance == 0.5:
            # No stop gradient
            value = kld(dist(post), dist(prior))
            ...
            value = kld(dist(sg(post)), dist(prior))
            pri = tf.reduce_mean(value)
            pos = tf.reduce_mean(dist(post)), kld(dist(sg(prior)))
...
class ConvEncoder(tools.Module):
    ...
    def __call__(self, obs):
        ...
        Conv = tfkl.Conv2D
        x = tf.reshape(obs['image_input'], (-1,) + tuple(obs['image_input'].shape[-3:]))
        ...
        shape = tf.concat([tf.shape(obs['image_input'])[:-3], [x.shape[-1]]], 0)
...
class ConvDecoder(tools.Module):
    ...
    def __init__(
            ...
            thin=True, strides=2, discrete=0):
        self._act = act
        self._depth = depth
        self._shape = shape
        self._kernels = kernels
    ...
    def __call__(self, features, dtype=None):
        ...
        if not self._discrete:
            ...
            return tfd.Independent(tfd.Categorical(logits=x), 2)
